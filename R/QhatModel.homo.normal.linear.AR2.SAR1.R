...
##' @export
QhatModel.homo.normal.linear.AR2.SAR1 <- setClass(
  # Set the name for the class
  "QhatModel.homo.normal.linear.AR2.SAR1",
  ...
  prototype=list(
    parameters =  new('parameters',c('mean.a0', 'mean.a1','mean.AR1','mean.AR2','mean.SAR1','std.a0'),c(1,1,1,1,1,1)),
...
setValidity("QhatModel.homo.normal.linear.AR2.SAR1", validObject)

# Initialise object
#setGeneric(name="initialize",def=function(.Object,input.data){standardGeneric("initialize")})
setMethod("initialize","QhatModel.homo.normal.linear.AR2.SAR1", function(.Object, input.data, use.truncated.dist=T, transition.graph=matrix(T,2,2),
  ...
  .Object@parameters = new('parameters', c('mean.a0', 'mean.a1', 'mean.AR1', 'mean.AR2','mean.SAR1','std.a0'), parameter.length)
...
setMethod(f="is.stationary",signature=c("QhatModel.homo.normal.linear.AR2.SAR1"),definition=function(.Object)
{
  ...
  AR.polynomial.coeffs = cbind(rep(1, .Object@nStates), -AR1, -AR2, -SAR1)
...
setMethod(f="getMean",signature=c("QhatModel.homo.normal.linear.AR2.SAR1","data.frame"),definition=function(.Object, data)
        {
          return(getMean.AR2.SAR1(.Object, data))
        }
)

# Calculate the transformed flow at the mean annual precip
setGeneric(name="getMean.AR2.SAR1",def=function(.Object, data) {standardGeneric("getMean.AR2.SAR1")})
setMethod(f="getMean.AR2.SAR1",signature=c("QhatModel.homo.normal.linear.AR2.SAR1","data.frame"),definition=function(.Object, data)
          {
            ...
            ncols.max = max(c(ncols.a0 ,ncols.a1,ncols.AR1,ncols.AR2,ncols.SAR1))
            ...
            for (i in 3:.Object@seasonal.step.size) {
            ...
            for (i in (.Object@seasonal.step.size+1):nrows) {
              Qhat.model[i,] <- precip.data[i,] * a1.est[i,] + a0.est[i,] + Qhat.model[i-1,] * AR1.est[i,] + Qhat.model[i-2,] * AR2.est[i,] +
                Qhat.model[i-.Object@seasonal.step.size,] * SAR1.est[i,]
            }
