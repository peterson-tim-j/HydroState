...
##' @export
QhatModel.homo.normal.linear.SAR1 <- setClass(
  # Set the name for the class
  "QhatModel.homo.normal.linear.SAR1",
  ...
  prototype=list(
    parameters =  new('parameters',c('mean.a0', 'mean.a1','mean.SAR1','std.a0'),c(1,1,1,1)),
...
setValidity("QhatModel.homo.normal.linear.SAR1", validObject)

# Initialise object
#setGeneric(name="initialize",def=function(.Object,input.data){standardGeneric("initialize")})
setMethod("initialize","QhatModel.homo.normal.linear.SAR1", function(.Object, use.truncated.dist=T,
                                                                    input.data, transition.graph=matrix(T,2,2),state.dependent.mean.a0=T,
                                                                    state.dependent.mean.a1=F, state.dependent.mean.SAR1=F,
  ...
  .Object@parameters = new('parameters', c('mean.a0', 'mean.a1', 'mean.SAR1','std.a0'), parameter.length)
...
setMethod(f="is.stationary",signature=c("QhatModel.homo.normal.linear.SAR1"),definition=function(.Object)
{
  # Get object parameter list
  parameters = getParameters(.Object@parameters)

  if (all(parameters$mean.SAR1 >-1 & parameters$mean.SAR1 <1)) {
    return(TRUE)
  } else {
    return(FALSE)
  }
...
setMethod(f="getMean",signature=c("QhatModel.homo.normal.linear.SAR1","data.frame"),definition=function(.Object, data) {
          return(getMean.SAR1(.Object, data))
  }
)

# Calculate the transformed flow at the mean annual precip
setGeneric(name="getMean.SAR1",def=function(.Object, data) {standardGeneric("getMean.SAR1")})
setMethod(f="getMean.SAR1",signature=c("QhatModel.homo.normal.linear.SAR1","data.frame"),definition=function(.Object, data)
          {
            ...
            ncols.max = max(c(ncols.a0 ,ncols.a1,ncols.SAR1))
            ...
            # Check the AR model is stationary.
            ...
            # Calculate the mean with the SAR1 seasonal componant.
            a0.est <- 100 * a0.est
            Qhat.model <- matrix(0, nrows, .Object@nStates)
            for (i in 1:.Object@seasonal.step.size) {
              Qhat.model[i,] <- precip.data[i,] * a1.est[i,] + a0.est[i,]
            }
            for (i in (.Object@seasonal.step.size+1):nrows) {
              Qhat.model[i,] <- precip.data[i,] * a1.est[i,] + a0.est[i,] + Qhat.model[i-.Object@seasonal.step.size,] * SAR1.est[i,]
            }
