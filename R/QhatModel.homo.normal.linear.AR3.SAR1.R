...
##' @export
QhatModel.homo.normal.linear.AR3.SAR1 <- setClass(
  # Set the name for the class
  "QhatModel.homo.normal.linear.AR3.SAR1",
  ...
  prototype=list(
    parameters =  new('parameters',c('mean.a0', 'mean.a1','mean.AR1','mean.AR2','mean.AR3','mean.SAR1','std.a0'),c(1,1,1,1,1,1,1)),
...
setMethod("initialize","QhatModel.homo.normal.linear.AR3.SAR1", function(.Object, input.data, use.truncated.dist=T, transition.graph=matrix(T,2,2),
  ...
  parameter.length <- as.numeric(c(state.dependent.mean.a0, state.dependent.mean.a1, state.dependent.mean.AR1,
                                   state.dependent.mean.AR2, state.dependent.mean.AR3, state.dependent.mean.SAR1, state.dependent.std.a0)) * (.Object@nStates-1) + 1

  # Set up model terms for mean and standard deviation.
  .Object@parameters = new('parameters', c('mean.a0', 'mean.a1', 'mean.AR1', 'mean.AR2','mean.AR3','mean.SAR1','std.a0'), parameter.length)
...
setMethod(f="is.stationary",signature=c("QhatModel.homo.normal.linear.AR3.SAR1"),definition=function(.Object)
{
  ...
  SAR1 <- parameters$mean.SAR1
  if (length(SAR1)==1)
    SAR1 = rep(SAR1, .Object@nStates)

  AR.polynomial.coeffs = cbind(rep(1, .Object@nStates), -AR1, -AR2, -AR3, -SAR1)
...
setMethod(f="getMean",signature=c("QhatModel.homo.normal.linear.AR3.SAR1","data.frame"),definition=function(.Object, data) {
  return(getMean.AR3.SAR1(.Object, data))
}
)

# Calculate the transformed flow at the mean annual precip
setGeneric(name="getMean.AR3.SAR1",def=function(.Object, data) {standardGeneric("getMean.AR3.SAR1")})
setMethod(f="getMean.AR3.SAR1",signature=c("QhatModel.homo.normal.linear.AR3.SAR1","data.frame"),definition=function(.Object, data)
          {
            ...
            ncols.SAR1 = length(parameters$mean.SAR1)
            nrows = length(data$Qhat.precipitation);
            ncols.max = max(c(ncols.a0 ,ncols.a1,ncols.AR1,ncols.AR2,ncols.AR3, ncols.SAR1))
            ...
            if (ncols.SAR1==1 || ncols.SAR1 ==.Object@nStates) {
              SAR1.est = matrix(rep(parameters$mean.SAR1,each=nrows),nrows,.Object@nStates);
            } else if (mean.SAR1<.Object@nStates) {
              stop(paste('The number of parameters for the a0 term of the mean model must must equal 1 or the number of states of ',.Object@nStates))
            }
            precip.data = matrix(data$Qhat.precipitation,nrows,.Object@nStates);

            # Calculate the mean with the AR componants and a seasonal componant.
            a0.est <- 100 * a0.est
            Qhat.model <- matrix(0, nrows, .Object@nStates)
            Qhat.model[1,] <- precip.data[1,] * a1.est[1,] + a0.est[1,]
            Qhat.model[2,] <- precip.data[2,] * a1.est[2,] + a0.est[2,] + Qhat.model[1,] * AR1.est[2,]
            Qhat.model[3,] <- precip.data[3,] * a1.est[3,] + a0.est[3,] + Qhat.model[2,] * AR1.est[3,] + Qhat.model[1,] * AR2.est[3,]
            for (i in 4:.Object@seasonal.step.size) {
              Qhat.model[i,] <- precip.data[i,] * a1.est[i,] + a0.est[i,] + Qhat.model[i-1,] * AR1.est[i,] + Qhat.model[i-2,] * AR2.est[i,] + Qhat.model[i-3,] * AR3.est[i,]
            }
            for (i in (.Object@seasonal.step.size+1):nrows) {
              Qhat.model[i,] <- precip.data[i,] * a1.est[i,] + a0.est[i,] + Qhat.model[i-1,] * AR1.est[i,] + Qhat.model[i-2,] * AR2.est[i,]
                              + Qhat.model[i-3,] * AR3.est[i,] + Qhat.model[i-.Object@seasonal.step.size,] * SAR1.est[i,]
            }
