...
Build the the 2-state annual hydroState model with `build()`. This model is the default and only requires input data. The input data can have gaps in flow or precipitation. A message will appear when gaps are in the data, but the model is still built. Handling the gaps are discussed in the `build` documentation. In order to adjust the default model, resort to the next vignette: `vignette("adjust.state.model")`. This `build()` command also accepts seasonal or monthly data if the input data contains these time-step. See vignette: `vignette("subAnnual.models")`.
