...
Below are four options to adjust the state model to better fit the model's residuals. For further details, see `build` in the [hydroState user manual](https://github.com/peterson-tim-j/HydroState/hydroState.pdf).
...
For this example, we compare the default state model and adjusted model. The default state model is first defined. Then an adjusted state model is created that assumes a gamma distribution in the residuals, 2-states, and auto-regressive lag of 2 time-steps. Note, a message appears after building the adjusted model with an `AR2` term because the first two streamflow observations are removed to allow the model to warm-up. See details in `build`.
...
Set the state names relative to a year in the record using `setInitialYear` and evaluat the flow states overtime of the adjusted model using `plot`. The year 1991 is selected as the initial year because there is not sufficient streamflow data for 1990 at this site (Bet Bet Creek - 407211).
...
#> ************************************
#> Calibrating Model: model.1State.truc.normal.boxcox.AR0.a0
...
#> model.2State.gamma.boxcox.AR2.a0        72.29543          10 2.269986e-01
#> model.2State.gamma.boxcox.AR3.a0        72.73455          11 1.822509e-01
#> model.2State.gamma.boxcox.AR0.a0        74.86469           8 6.282261e-02
#> model.2State.gamma.boxcox.AR1.a0        76.77487           9 2.417276e-02
#> model.3State.gamma.boxcox.AR0.a0        81.88396          15 1.878888e-03
#>...
```
