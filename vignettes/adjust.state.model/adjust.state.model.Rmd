...
```r
# Default state model
default.state.model = select.stateModel(input.data = streamflow_annual_407211,
...
gamma.state.model = select.stateModel(input.data = streamflow_annual_407211,
                              parameters = list('a0','a1','std'),
                              state.shift.parameters = list('a0','std'),
...
The example continues building two hydroState models, one for the default state model and the other for the adjusted state model.

```r
# Build hydroState model for default model
default.model.annual = buildModel(input.data = streamflow_annual_407211,
                    stateModel = default.state.model)

# Build hydroState model for adjusted state model
gamma.model.annual = buildModel(input.data = streamflow_annual_407211,
                    stateModel = gamma.state.model)
```
...
plot.residuals(gamma.model.annual, do.pdf = T, ID ='407211')
...
# plot all four plots
plot.states(gamma.model.annual)
...
How do we know which state model is the best given our observations? Rather than manually testing every adjustment in the state model, the `buildModelAll` function builds every possible state model combination (each distribution, auto-correlation terms, and 1 to 3-states) while the intercept, `a_1` and error `std` are considered `state.shift.parameters`. The function performs similarly to `buildModel` and can easily replace this function in the workflow. After being built, the `fitModel` function is performed just as before, and the best model is based on the lowest AIC. 

Below is a an example of how to set up `buildModelAll`, but the built models are not fitted in this example due to time constraints. It is best to run `fitModel` in parallel when fitting all built models to save computation time. 
...
# all.models = fitModel(all.models, pop.size.perParameter = 10, max.generations = 500, do.parallel = TRUE)
