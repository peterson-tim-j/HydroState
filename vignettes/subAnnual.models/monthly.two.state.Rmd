---
title: "Monthly model and distributions"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Monthly model and distributions}
...
## Load required data
Monthly rainfall-runoff models require a data frame with catchment average runoff and precipitation for each month. Load the data into the environment. Ensure there are four columns named "year", "month", "flow", and "precipitation", and verify the units for flow and precipitation are the same ('mm', 'in', etc.). Note the data provided are in water years for Victoria, Australia (i.e. year 1950 is March-1950 to February-1951)

```r
data(streamflow_monthly)

# check input data
head(streamflow_monthly)
...
Before building the model, select a state model using `select.stateModel` that better suites the distribution or auto-correlation within the residuals. This function provides several options to better define the rainfall-runoff relationship. Below are three options to adjust the state model in order for the residuals to be uniform and normally distributed with no trends or auto-correlation. For further details, see help page for `select.stateModel`.

1) `parameters` - enter $AR1$, $AR2$, or $AR3$ to assume auto-correlation of 1, 2, or 3 time lags 
2) `seasonal.parameters` - assume a `parameter` is best modeled as a sinusoidal function representing seasonal trends. The input accepts any default parameter: $a_1$, $a_0$, or $std$. Typically, assuming seasonal trends in only the intercept, $a_0$, explains trends in the model. !!!!!!!!!!!!!!!! Note, `seasonal.parameters` is only available for monthly or seasonal models, not annual models. 
3) `error.distribution` - adjust the assumed distribution of the residuals. `truc.normal`, `normal`, and `gauss` are available. Note, `gauss` is only available for monthly and seasonal models. 
...
```r
# Default state model
default.state.model = select.stateModel(input.data = streamflow_monthly,
...
AR1.state.model = select.stateModel(input.data = streamflow_monthly,
...
AR1.model = buildModel(input.data = streamflow_monthly,
...
```r
AR1.model = fitModel(AR1.model, pop.size.perParameter = 10, max.generations = 500)
```
...
# plot all four plots
model.states = plot.states(default.model.annual)
