...
```r
# Default state model
default.state.model.seasonal = select.stateModel(input.data = streamflow_seasonal_221201,
...
adjusted.state.model.monthly = select.stateModel(input.data = streamflow_monthly_221201,
                              parameters = list('a0','a1','std','AR3'),
                              seasonal.parameters = list('a0','a1'),
                              error.distribution = 'gamma',
...
model.seasonal.default = buildModel(input.data = streamflow_seasonal_221201,
...
model.monthly.adjusted = buildModel(input.data = streamflow_monthly_221201,
                      stateModel = adjusted.state.model.monthly,
                      data.transform = select.transform(func = 'log',input.data = streamflow_monthly_221201))
```
...
model.seasonal.default.all = fitModel(model.seasonal.default.all, pop.size.perParameter = 10, max.generations = 10000, doParallel = TRUE)
...
plot.residuals(model.monthly.adjusted)
...
```r
<!-- all.models.221201.seasonal = buildModelAll(input.data = streamflow_seasonal_221201) -->

<!-- all.models.221201.seasonal = fitModel(all.models.221201.seasonal) -->
