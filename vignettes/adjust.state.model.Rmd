...
The default state model from the [Getting Started](articles/hydroState.html) article might not adequately fit the observed rainfall-runoff relationship. In order for the state shifts to be statistically adequate and informative, the model's residuals must adhere to the required checks in [plot(`pse.residuals = TRUE`)](reference/plot.html) (uniform and normally distributed with no trends or auto-correlation). This vignette introduces a suite of options to adjust the default state model when model residuals do not meet the checks. Most often, this requires adjusting the shape of the residual distribution (`error.distribution`), applying auto-correlation terms (`parameters`), or possibly allowing for change in different model parameters (`state.shift.parameters`).

This vignette works through an example comparing the default model with an adjusted model. It concludes with the introduction of [build.all()](reference/build.all.html) function that systematically builds all possible adjustments in the state model in order to identify the best performing model. 
...
For this example, we compare the default state model and adjusted model. The default state model is first defined. Then an adjusted state model is created that assumes a gamma distribution in the residuals, 2-states, and auto-regressive lag of 2 time-steps. Note, a message appears after building the adjusted model with an `AR2` term because the first two streamflow observations are removed to allow the model to warm-up. See details in [build()](reference/build.html).
...
```
<center>
![Comparison between the residual plots of the default annual model with a truncated normal distribution and adjusted annual model with a gamma distribution and two auto-correlation lags. p-value greater than 0.05 in adjusted model suggests gamma model better suites the residual distribution at this site (407211)](default.vs.gamma.annual.two.state.residuals.407211.png){width=100%}
<\center>

## Evaluate the state-shifts
Set the state names relative to a year in the record using `setInitialYear()` and evaluat the flow states overtime of the adjusted model using ([plot()](reference/plot.html)). The year 1991 is selected as the initial year because there is not sufficient streamflow data for 1990 at this site (Bet Bet Creek - 407211).
