...
For comparing the default state model and adjusted model, the default state model is first defined. Then an adjusted state model is created that assumes a gamma distribution in the residuals.  
...
Build two hydroState models, one for the default state model and the other for the adjusted state model.
...
Set the state names relative to a year in the record using `setInitialYear` and evaluat the flow states overtime of the adjusted model using `plot.states`. The year 1991 is selected as the initial year because there is not sufficient streamflow data for 1990.
...
# plot all four plots
model.states = plot.states(gamma.model.annual)
