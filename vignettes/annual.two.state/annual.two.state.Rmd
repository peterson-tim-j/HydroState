...
Build the the 2-state annual hydroState model with `build`. This model is the default and only requires input data. The input data can have gaps in flow or precipitation. A message will appear when gaps are in the data, but the model is still built. Handling the gaps are discussed in the `build` documentation. In order to adjust the default model, resort to the next vignette: [adjust.state.model](https://github.com/peterson-tim-j/HydroState/tree/master/vignettes/adjust.state.model/adjust.state.model.html). This `build` command also accepts seasonal or monthly data if the input data contains these time-step. See vignette:  [subAnnual.models](https://github.com/peterson-tim-j/HydroState/tree/master/vignettes/subAnnual.models/subAnnual.models.html). 
